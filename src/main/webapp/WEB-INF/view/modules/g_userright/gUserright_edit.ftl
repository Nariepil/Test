...
<body>
	<div class="form">
		<form class="editForm" target="_parent" id="gUserrightEditForm" action="${request.getContextPath()}/g_userright/gUserright/updateGUserright" method="post">
			<input type="hidden" name="id" value="${gUserright.id}"/>
			<table border="0" cellspacing="0" cellpadding="0" width="100%" style="margin-top:20px;" class="boxTable boxmar">
	            ...
	            <tr>
	            	<td align="right" style="width:80px"><span class="check">*</span>用户编号，对应g_user表id：</td>
	      			<td style="width:150px">
	      				<input name="userId" id="userId" class="cword" value="${(gUserright.userId)!''}"  datatype="s2-16" nullmsg="请输入用户编号，对应g_user表id！" errormsg="用户编号，对应g_user表id至少2个字符,最多16个字符！"/>
	            	</td>
	            	<td>
	            ...
	            <tr>
	            	<td align="right" style="width:80px"><span class="check">*</span>权限编号，对应g_right表id：</td>
	      			<td style="width:150px">
	      				<input name="rightId" id="rightId" class="cword" value="${(gUserright.rightId)!''}"  datatype="s2-16" nullmsg="请输入权限编号，对应g_right表id！" errormsg="权限编号，对应g_right表id至少2个字符,最多16个字符！"/>
	            	</td>
	            	<td>
    					<div class="Validform_checktip"></div>
    				</td>
